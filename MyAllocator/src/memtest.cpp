...
int main(int argc, char * argv[]) {
  ...
  size_t block_size = 128; /* 128B -- DEFAULT! */
  size_t mem_size = 1024 * 512; /* 512KB -- DEFAULT! */
  int opt;
  ...
  while( ( opt = getopt (argc, argv, "b:s:") ) != -1 )
  {
      switch(opt)
      {
          case 'b':
        	  ...
        	  break;
          case 's':
        	  mem_size = 1024 * atoi(optarg);
              break;
      }
  }
  cout<<"Your block size: "<< block_size<< " bytes. "<<endl;
  cout<<"Your mem size: "<< mem_size << " bytes. "<<endl;
